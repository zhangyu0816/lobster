...
public class WheelRecycle {
	 ...
	 * @param firstItem the number of first item_check_add_balance in layout
	 * @param range the range of current wheel items 
	 * @return the new value of first item_check_add_balance number
	 */
	public int recycleItems(LinearLayout layout, int firstItem, ItemsRange range) {
		...
		for (int i = 0; i < layout.getChildCount();) {
			if (!range.contains(index)) {
				...
				if (i == 0) { // first item_check_add_balance
					firstItem++;
				}
			} else {
				i++; // go to next item_check_add_balance
			}
			index++;
		}
		return firstItem;
	...
	/**
	 * Gets item_check_add_balance view
	...
	/**
	 * Gets empty item_check_add_balance view
	...
	/**
	 * Adds view to cache. Determines view type (item_check_add_balance view or empty one) by index.
	 * @param view the view to be cached
